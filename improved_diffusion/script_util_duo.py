...
from . import gaussian_diffusion_duo as gd
from .respace_duo import SpacedDiffusion, space_timesteps
from .unet import  UNetModel

NUM_CLASSES = 2

def sr_create_model(**kwargs):
    pass
...
def model_and_diffusion_defaults():
    ...
    """
    return dict(
        image_size=512,  # Updated to match the sampling part
        num_channels=128,
        num_res_blocks=3,  # Updated to match the sampling part
        num_heads=4,
        num_heads_upsample=-1,
        attention_resolutions="16,8",
        dropout=0.0,
        ...
        sigma_small=False,
        class_cond=False,
        diffusion_steps=1000,  # Matching the sampling part
        noise_schedule="cosine",  # Matching the sampling part
        timestep_respacing="",  # Matching the sampling part
        use_kl=False,
        predict_xstart=False,
        rescale_timesteps=True,  # Matching the sampling part
        rescale_learned_sigmas=True,
        use_checkpoint=False,
        use_scale_shift_norm=True,
    )
...
def create_model_and_two_diffusion(
    ...
    diffusion_two = create_gaussian_diffusion(
        steps=diffusion_steps,
        learn_sigma=learn_sigma,
        sigma_small=sigma_small,
        noise_schedule=noise_schedule,
    ...
    return model, diffusion, diffusion_two
...
def sr_create_model_and_diffusion(
    ...
    model = sr_create_model(  # type: ignore 
        large_size,
        small_size,
        num_channels,
        num_res_blocks,
...
#### TESTING WITH RANDOM NOISE: FAILED
